...
func (w *Worker) SetupJob(j *Job) error {
	...
	j.GitFolder = filepath.Join("jobs", j.ID, "git")
	err = os.MkdirAll(j.GitFolder, 0755)
	if err != nil {
		return err
	}
	return nil
...
func (w *Worker) CloneAndMerge(j *Job) error {
	...
	_, err := git.PlainCloneContext(j.ctx, j.GitFolder, false,
		&git.CloneOptions{
...
func (w *Worker) ProcessJob(j *Job) {
	...
	err = ExecScript(j.ctx, j.Script, j.Event)
	if err != nil {
		exit := &exec.ExitError{}
